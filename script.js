...
function ingresarNumero(numero) {
  ...
  if (valorActual === "0" && numero !== ".") { valorActual = numero; }
  else { valorActual += numero; }
}
...
function calcular() {
  ...
  switch (operador) {
    ...
    case "dividir": resultado = b === 0 ? "Error" : a / b; break;
    default: return;
...
function limpiar() { valorActual = "0"; valorAnterior = ""; operador = null; }
function borrar() { valorActual = valorActual.length > 1 ? valorActual.slice(0, -1) : "0"; }
function porcentaje() { valorActual = (parseFloat(valorActual) / 100).toString(); }
 
document.querySelectorAll(".btn-numero").forEach((btn) => {
  btn.addEventListener("click", () => { ingresarNumero(btn.dataset.numero); actualizarPantalla(); });
});
document.querySelectorAll(".btn-operador").forEach((btn) => {
  btn.addEventListener("click", () => { elegirOperador(btn.dataset.accion); actualizarPantalla(); });
});
...
document.querySelector(".btn-igual").addEventListener("click", () => { calcular(); actualizarPantalla(); });
